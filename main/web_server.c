...
static void http_server(struct netconn *conn) {
	...
	const static char JPEG_HEADER[] = "HTTP/1.1 200 OK\nContent-type: image/jpeg\n\n";
	...
	if(err==ERR_OK) {
		...
		if(buf) {
			...
			// default page websocket
			...
			else if(strstr(buf,"GET /picture.jpeg ")) {
				char *filename = "/spiffs/picture.jpeg";
				ESP_LOGI(TAG,"Sending picture.jpeg");
				struct stat statBuf;
				...
				image_buffer = malloc(filesize);
				if (image_buffer == NULL) {
					ESP_LOGE(TAG, "malloc fail. image_buffer %d", filesize);
					return;
				}
				FILE * fp;
				...
				netconn_write(conn,JPEG_HEADER,sizeof(JPEG_HEADER)-1, NETCONN_NOCOPY);
				//netconn_write(conn,favicon_ico_start,favicon_ico_len, NETCONN_NOCOPY);
				netconn_write(conn,image_buffer,filesize, NETCONN_NOCOPY);
				netconn_close(conn);
				netconn_delete(conn);
				netbuf_delete(inbuf);
				free(image_buffer);
